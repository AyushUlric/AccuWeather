...
class StartScreen(Screen):
	def register(self):
		...
		if x["cod"] != "404": 
			...
			titleA = f"Todays Weather in {city_name} -({datetoday})"
			...
			Buttons = [close_button]
			close_button = MDFlatButton(text='Close', on_release=self.close_dialog)
			self.dialog = MDDialog(title = f"Error", text= f"Please Recheck Your Entry And Make Sure You have An Active Internet Connection", size_hint=(0.95,1),
			buttons = Buttons,
			)
			self.dialog.open()
